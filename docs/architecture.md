...
- `scripts/`：模型资产静态校验。
- `tools/blender/`：可复现的 MPFB 成年女性多服装、多交付层级候选目录生成管线。
...
页面的模型目录是“预览选择层”，课程中的 `modelAsset` 是“正式课程资产层”。用户可以选择任意 `technical-review` 候选进行比较，但不会因此修改课程批准状态。默认预览选择休闲装移动版，课程正式资产仍由 Schema 和审核记录控制。
...
- 路径不能逃逸 `public/`；
- 文件大小与目录元数据一致，且不超过该交付层级预算；
- SHA-256 与清单、`SHA256.txt` 和生成目录摘要一致；
- GLB magic、版本、声明长度和 JSON 块有效；
- glTF 2.0、场景、节点、网格和蒙皮存在；
- `boneMap` 的每个实际节点名存在于 GLB；
- 来源、许可证、四视图和候选报告存在；
- 移动版持续满足 12 MiB 与 1024 贴图上限。

MPFB 目录包括休闲装移动版、休闲装原始质量版和运动装原始质量版。Khronos/Cesium `RiggedFigure` 只用于加载器、蒙皮、动画、完整性和回退 QA，不是正式教学人物。
...
- `modelId` 与模型资产 ID 一致；
- 模型资产状态为 `approved`，且呈现类型必须是 `neutral-fully-clothed`；
- 姿势状态为 `approved`；
- 存在独立审核者；
...
- 低性能时自动降低 DPR 和阴影质量。
- 二维分步图完全不依赖 Three.js。
- MPFB 移动候选限制为 12 MiB 和 1024 贴图；原始质量候选使用独立预算并在界面提示下载成本。
- 后续仍可评估 Meshopt/Draco、WebP/KTX2、LOD 和更多真机内存测试，但这些优化不再阻塞候选比较。
